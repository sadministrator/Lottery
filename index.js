...
//node index.js Lottery.json $YOURPRIVATEKEY
//OR
//node index.js Lottery.json and replace DEPLOYER_PRIVATE_KEY to your private key with funds on nervos Layer 2.
...
if (!contractName) {
    throw new Error(`No compiled contract specified to deploy. Please put it in "./build/contracts" directory and provide its name as an argument to this program, eg.: "node index.js Lottery.json $YOURPRIVATEKEY"`);
}
...
const DEPLOYER_PRIVATE_KEY = process.argv.slice(2)[1];//Or just replace this with your Ethereum private key with funds on nervos Layer 2.
